...
- Not a flag: the academic style standard in `references/style-standard.md` (section conventions in `references/paper-format.md`) applies to every run; workers and the synthesis agent write to it, and the Phase 4.5 filler grep is a hard gate. `--human-readable` adds surface rules on top of it.
